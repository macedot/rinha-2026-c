...
config_t config_load(void) {
    ...
    cfg.use_tcp = env_bool("LISTEN_TCP", 0);
    cfg.port = (uint16_t)env_int("PORT", 9999, 1, 65535);
    cfg.host = env_str("HOST", "0.0.0.0");
    ...
    cfg.tcp_nodelay = env_bool("TCP_NODELAY", 1);
    cfg.reuse_port = env_bool("SO_REUSEPORT_ENABLED", 1);
    return cfg;
}
